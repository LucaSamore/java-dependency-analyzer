...
class DependencyGraphPanel : JPanel() {
  ...
  private val nodes = mutableMapOf<String, Pair<Double, Double>>() // name -> (x, y)
  ...
  private fun buildGraph() {
    // Create edge
    val uniqueClasses = dependencies.flatMap { listOf(it.from, it.to) }.toSet()

    val oldPositions = HashMap(nodes)
    nodes.clear()
    nodeColors.clear()
    ...
    uniqueClasses.forEach { className ->
      nodes[className] = oldPositions[className] ?: Pair(
    ...
    for (className in uniqueClasses) {
      val pkg = className.substringBeforeLast(".", "default")
      if (pkg !in packageColors) {
        packageColors[pkg] = Color(
          150 + random.nextInt(80),
          150 + random.nextInt(80),
          150 + random.nextInt(80)
        )
      }
      nodeColors[className] = packageColors[pkg]!!
    }

    if (selectedNode != null && selectedNode !in nodes) {
      selectedNode = null
  ...
  private fun getNodeAt(x: Int, y: Int): String? {
    ...
    return nodes.entries.firstOrNull { (_, pos) ->
      val dx = tx - pos.first
  ...
  override fun paintComponent(g: Graphics) {
    ...
    val selectedConnections = if (selectedNode != null) {
      dependencies.filter { it.from == selectedNode || it.to == selectedNode }
    } else emptyList()

    dependencies.forEach { dep ->
      val sourcePos = nodes[dep.from] ?: return@forEach
      val targetPos = nodes[dep.to] ?: return@forEach

      val (x1, y1) = sourcePos
      val (x2, y2) = targetPos

      g2d.color = if (dep in selectedConnections) Color(50, 100, 200) else Color(180, 180, 180, 150)
    ...
    nodes.forEach { (name, pos) ->
      ...
      g2d.fillOval((x-nodeRadius).toInt(), (y-nodeRadius).toInt(), (nodeRadius*2).toInt(), (nodeRadius*2).toInt())
      g2d.color = Color.BLACK
      g2d.drawOval((x-nodeRadius).toInt(), (y-nodeRadius).toInt(), (nodeRadius*2).toInt(), (nodeRadius*2).toInt())
