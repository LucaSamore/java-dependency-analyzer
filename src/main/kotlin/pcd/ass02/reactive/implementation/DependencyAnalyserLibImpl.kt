...
class DependencyAnalyserLibImpl(private val parser: ParserLib) : DependencyAnalyserLib {
  ...
  override val dependencies: Flowable<List<Dependency>> =
      dependenciesProcessor.onBackpressureBuffer()
  ...
  override fun analyse(projectPath: String) {
    ...
    analysisSubscription =
        parser
            ...
            .map { dependencies ->
              val uniqueClasses =
                  dependencies.flatMap { listOf(it.sourceClass, it.targetClass) }.toSet()

              Pair(dependencies, uniqueClasses)
            }
            .observeOn(Schedulers.io())
            .subscribe(
                { (dependencies, uniqueClasses) ->
                  dependenciesProcessor.onNext(dependencies)
                  statusProcessor.onNext(
                      "Analyzing: ${uniqueClasses.size} classes, ${dependencies.size} dependencies")
                },
                { error -> statusProcessor.onNext("Analysis failed: ${error.message}") },
